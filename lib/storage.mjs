...
export async function createStorage(env, directory) {
  if (env.STORAGE_MODE === 'supabase') {
    ...
    return {
      ...
      async listDays(){const results=[];for(let offset=0;;offset+=1000){const page=await(await request(`/rest/v1/thermalnote_days?order=date.desc&limit=1000&offset=${offset}`)).json();results.push(...page);if(page.length<1000)break;}return results;},
      async saveDay(date,data,expected){try{const row=await(await request('/rest/v1/rpc/thermalnote_day_save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({day_date:date,day_data:data,expected_version:expected})})).json();return Array.isArray(row)?row[0]:row;}catch(error){throw error.status===409?new ConflictError('This day changed in another window. Reopen it to load the latest sets.'):error;}},
  ...
  return {
    ...
    async save(note,expected){
      ...
      if(expected===0){try{db.prepare('INSERT INTO notes (id,title,content,marks,created_at,updated_at,version) VALUES (?,?,?,?,?,?,1)').run(note.id,note.title,note.content,marks,now,now);}catch(e){if(e.message.includes('UNIQUE'))throw new ConflictError();throw e;}}
      else{const result=db.prepare('UPDATE notes SET title=?,content=?,marks=?,updated_at=?,version=version+1 WHERE id=? AND version=? AND deleted_at IS NULL').run(note.title,note.content,marks,now,note.id,expected);if(!result.changes)throw new ConflictError();}
    ...
    async listDays(){return db.prepare('SELECT * FROM days ORDER BY date DESC').all().map(row=>({...row,data:JSON.parse(row.data)}));},
    async saveDay(date,data,expected){
      const stamp=new Date().toISOString(),text=JSON.stringify(data);
      if(expected===0){try{db.prepare('INSERT INTO days (date,data,updated_at,version) VALUES (?,?,?,1)').run(date,text,stamp);}catch(e){if(e.message.includes('UNIQUE'))throw new ConflictError('This day changed in another window. Reopen it to load the latest sets.');throw e;}}
      else{const result=db.prepare('UPDATE days SET data=?,updated_at=?,version=version+1 WHERE date=? AND version=?').run(text,stamp,date,expected);if(!result.changes)throw new ConflictError('This day changed in another window. Reopen it to load the latest sets.');}
      const row=db.prepare('SELECT * FROM days WHERE date=?').get(date);return {...row,data:JSON.parse(row.data)};
    },
