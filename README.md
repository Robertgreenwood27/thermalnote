...
- **Deleting takes two presses.** Backspace or Delete never takes a card by accident. Deleting a blank line between cards removes only the line. When the next press would take a card, the card turns red and dashed and says **Backspace again to delete**; press again to delete it, or do anything else to leave it alone. Selecting across cards works the same way.
- **Copying a card.** **⧉ Copy** in a card's top strip puts its front and back on the clipboard, labelled and headed with the note's title, ready to paste into a chatbot and talk through. It copies both sides even while the back is hidden. The drill has the same button as **⧉ Copy card**.
...
- **Drilling is its own screen.** **◈ n warm** in the top bar counts every red, orange, and yellow card across every note. Tapping it opens the drill: one card at a time in a pop-up over the page, in random order so where a card sits in a note is never the cue. Space or **Reveal** shows the answer under the prompt; answer Forgot, Hard, or Got it (keys 1, 2, 3). A forgotten card comes back a few cards later in the same drill. **Open in note** jumps to the card where it lives, and Esc or × leaves.
- **Both directions.** The **F→B** switch beside the warm counter flips the drill to **B→F**: each card shows its back and you recall the front. Each direction keeps its own confidence, since knowing the answer from the prompt says little about the prompt from the answer, so a card can be cool one way and red the other. The warm counter, the note list, and the card borders all follow the chosen direction, and the choice is remembered. Cards with an empty back, and passages marked before cards existed, only take part front to back.
