...
create or replace function public.thermalnote_save(note_id uuid, note_title text, note_content text, expected_version integer)
...
begin
  if expected_version = 0 then
    begin
      insert into public.thermalnote_notes(id,title,content) values(note_id,note_title,note_content) returning * into saved;
    exception when unique_violation then
      raise exception 'Note version conflict' using errcode = 'TN409';
    end;
  else
    update public.thermalnote_notes set title=note_title,content=note_content,updated_at=now(),version=version+1
...
$$;
revoke all on function public.thermalnote_save(uuid,text,text,integer) from public, anon, authenticated;
grant execute on function public.thermalnote_save(uuid,text,text,integer) to service_role;
