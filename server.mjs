...
const types={'.html':'text/html; charset=utf-8','.css':'text/css; charset=utf-8','.js':'text/javascript; charset=utf-8','.svg':'image/svg+xml','.png':'image/png','.jpg':'image/jpeg','.webp':'image/webp','.gif':'image/gif','.avif':'image/avif'};
...
async function body(req,max=32*1024*1024){let size=0;const chunks=[];for await(const chunk of req){size+=chunk.length;if(size>max)throw fail('This upload is too large. Images can be up to 12 MB.',413);chunks.push(chunk);}try{return JSON.parse(Buffer.concat(chunks).toString());}catch{throw fail('The request was not valid.');}}
...
export async function createApp({env=process.env,dataDirectory=env.DATA_DIR||path.join(project,'data')}={}){
  if(!env.APP_PASSWORD_HASH)throw Error('Set APP_PASSWORD_HASH in .env before starting Thermalnote. See README.md.');
  const storage=await createStorage(env,dataDirectory);
  const sessions=new Map(), attempts=new Map();
  const sessionAge=7*24*60*60*1000;
  const clean=setInterval(()=>{const now=Date.now();for(const[token,expires]of sessions)if(expires<now)sessions.delete(token);for(const[ip,record]of attempts)if(record.until<now)attempts.delete(ip);},60000).unref();
  const server=http.createServer(async(req,res)=>{
    res.setHeader('Cache-Control','no-store');res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Referrer-Policy','no-referrer');res.setHeader('X-Frame-Options','DENY');
    res.setHeader('Content-Security-Policy',"default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' https: blob:; connect-src 'self'; object-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'");
    try{
      ...
      const secure=env.APP_ORIGIN?.startsWith('https:')||env.COOKIE_SECURE==='true';
      const cookie=(value,maxAge)=>`thermalnote_session=${value}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${maxAge}${secure?'; Secure':''}`;
      if(req.method!=='GET'&&req.method!=='HEAD'){
        const allowed=env.APP_ORIGIN||`http://${req.headers.host}`;
        if(req.headers.origin!==allowed||req.headers['sec-fetch-site']==='cross-site')throw fail('This request came from another website.',403);
        if(!req.headers['content-type']?.startsWith('application/json'))throw fail('Use a JSON request.',415);
      }
      if(pathname==='/api/login'&&req.method==='POST'){
        const input=await body(req,4096),ip=req.socket.remoteAddress;
        let record=attempts.get(ip);if(!record||record.until<Date.now()){record={count:0,until:Date.now()+15*60*1000};attempts.set(ip,record);}
        if(record.count>=10)throw fail('Too many attempts. Please try again in 15 minutes.',429);
        record.count++;
        if(typeof input.username!=='string'||typeof input.password!=='string'||input.password.length>1024||!checkPassword(input.password,env.APP_PASSWORD_HASH)||input.username!==(env.APP_USERNAME||'Arris'))throw fail('The username or password isn’t right.',401);
        attempts.delete(ip);const fresh=randomBytes(32).toString('hex');sessions.set(fresh,Date.now()+sessionAge);res.setHeader('Set-Cookie',cookie(fresh,sessionAge/1000));return json(res,200,{username:env.APP_USERNAME||'Arris',storage:storage.mode});
      }
      if(pathname.startsWith('/api/')||pathname.startsWith('/media/')){
        if(!authenticated)throw fail('Please sign in again. Your unsaved draft is still here.',401);
        if(pathname==='/api/session'&&req.method==='GET')return json(res,200,{username:env.APP_USERNAME||'Arris',storage:storage.mode});
        if(pathname==='/api/logout'&&req.method==='POST'){sessions.delete(token);res.setHeader('Set-Cookie',cookie('',0));return json(res,200,{ok:true});}
        ...
        if(pathname.startsWith('/media/')&&req.method==='GET'){
          const name=pathname.slice(7);if(!/^[\da-f-]{36}\.(png|jpg|webp|gif|avif)$/.test(name))throw fail('Image not found.',404);const bytes=await storage.getImage(name);res.writeHead(200,{'Content-Type':types[path.extname(name)],'Cache-Control':'private, no-store'});return res.end(bytes);
        }
        throw fail('Not found.',404);
      }
      const publicFiles={'/':'index.html','/index.html':'index.html','/app.js':'app.js','/heat.js':'heat.js','/style.css':'style.css','/favicon.svg':'favicon.svg'};
  ...
  server.on('close',()=>{clearInterval(clean);storage.close();});return {server,storage};
}
